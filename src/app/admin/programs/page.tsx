...
export default function ProgramsAdminPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) { setEditing(null); fetchPrograms(); }
  ...
  if (loading && programs.length === 0) return <div className="flex items-center justify-center h-screen"><Loader2 className="animate-spin text-[#002d56]" size={48} /></div>;

  return (
    <div className="p-8 font-sans">
      <div className="flex justify-between items-center mb-10">
        <div>
          <h1 className="text-3xl font-black text-[#002d56] mb-2 uppercase tracking-tighter">Academic Programs</h1>
          <p className="text-slate-400 font-medium uppercase text-xs tracking-widest flex items-center gap-2">
            Admin <ChevronRight size={14} /> Programs
          </p>
        </div>
        <button 
          onClick={() => setEditing({ title: '', shortDesc: '', image: '', isActive: true })}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {programs.map((prog) => (
          <div key={prog._id} className="bg-white rounded-sm shadow-xl border border-slate-100 overflow-hidden">
            <div className="h-48 bg-slate-100 relative">
              {prog.image && <img src={prog.image} alt={prog.title} className="w-full h-full object-cover" />}
            </div>
            <div className="p-6">
              <h3 className="text-xl font-black text-[#002d56] uppercase tracking-tighter mb-2">{prog.title}</h3>
              <p className="text-sm text-slate-500 mb-4">{prog.shortDesc}</p>
              <div className="flex justify-between items-center border-t border-slate-100 pt-4">
                <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${prog.isActive ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'}`}>
                  {prog.isActive ? 'Active' : 'Hidden'}
                </span>
                <div className="flex gap-2">
                  <button onClick={() => setEditing(prog)} className="p-2 bg-slate-50 text-[#002d56] rounded-sm hover:bg-[#ffcc00]"><Edit2 size={16} /></button>
                  <button onClick={() => handleDelete(prog._id)} className="p-2 bg-red-50 text-red-500 rounded-sm hover:bg-red-500 hover:text-white"><Trash2 size={16} /></button>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-[1100] flex items-center justify-center p-6 bg-[#002d56]/80 backdrop-blur-sm">
          <div className="bg-white w-full max-w-2xl rounded-sm shadow-2xl overflow-hidden">
            <div className="bg-[#002d56] p-8 flex justify-between items-center">
              <h2 className="text-white text-2xl font-black uppercase tracking-tighter">{editing._id ? 'Edit Program' : 'New Program'}</h2>
              <button onClick={() => setEditing(null)} className="text-white/60 hover:text-white"><X size={28} /></button>
            </div>
            
            <form onSubmit={handleSave} className="p-10 space-y-6">
              <div>
                <label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Program Title</label>
              ...
              <div>
                ...
                <input required type="text" value={editing.shortDesc || ''} onChange={e => setEditing({...editing, shortDesc: e.target.value})} className="w-full px-5 py-4 rounded-sm border border-slate-200 focus:border-[#ffcc00] outline-none text-sm font-medium" />
              </div>
              <div>
                <label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2 flex items-center gap-2"><Camera size={14} /> Program Image</label>
